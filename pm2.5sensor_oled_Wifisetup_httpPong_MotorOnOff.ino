#include <Wire.h>
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>
#include "PMS.h"
#include <WiFi.h>
#include <WebServer.h>

// --- WIFI CONFIGURATION ---
const char* wifiSsid = "SIT-HACKATHON";     
const char* wifiPassword = "sithackathon"; 

// --- MOTOR CONFIGURATION ---
const int MOTOR_PIN = 4; // Connect Relay or Motor Driver IN to GPIO 4

// --- WEB SERVER CONFIGURATION ---
WebServer server(8010); 

// --- OLED CONFIGURATION ---
#define SCREEN_WIDTH 128
#define SCREEN_HEIGHT 64
#define OLED_RESET    -1 
Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET);

// --- PMS SENSOR CONFIGURATION ---
PMS pms(Serial2);
PMS::DATA data;

// Global variables
int currentPM25 = 0;
int currentPM10 = 0;
String motorStatus = "OFF";

void setup() {
  Serial.begin(115200);
  
  // 1. Setup Motor Pin
  pinMode(MOTOR_PIN, OUTPUT);
  digitalWrite(MOTOR_PIN, LOW); // Start with motor OFF
  
  // 2. Initialize PMS Sensor
  Serial2.begin(9600, SERIAL_8N1, 16, 17);

  // 3. Initialize OLED
  if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { 
    Serial.println(F("Screen failed to start"));
    for(;;);
  }
  
  // Wi-Fi Connection UI
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(0, 10);
  display.println("Connecting to WiFi...");
  display.display();

  WiFi.begin(wifiSsid, wifiPassword);
  
  int retry_count = 0;
  while (WiFi.status() != WL_CONNECTED && retry_count < 20) {
    delay(500);
    display.print(".");
    display.display();
    retry_count++;
  }
  
  display.clearDisplay();
  display.setCursor(0, 10);

  if(WiFi.status() == WL_CONNECTED) {
    Serial.println("\nWiFi Connected!");
    Serial.print("IP Address: ");
    Serial.println(WiFi.localIP()); 
    
    display.println("WiFi Connected!");
    display.print("IP: ");
    display.println(WiFi.localIP());
    
    // --- SERVER ENDPOINTS ---

    // 1. Root Page (Status)
    server.on("/", HTTP_GET, []() {
      String message = "Air Quality Monitor & Dispenser\n\n";
      message += "PM 2.5: " + String(currentPM25) + " ug/m3\n";
      message += "PM 10:  " + String(currentPM10) + " ug/m3\n";
      message += "Motor Status: " + motorStatus + "\n\n";
      message += "Usage:\n";
      message += "/motor/on  -> Turn ON\n";
      message += "/motor/off -> Turn OFF";
      server.send(200, "text/plain", message);
    });

    // 2. Turn Motor ON
    server.on("/motor/on", HTTP_GET, []() {
      digitalWrite(MOTOR_PIN, HIGH);
      motorStatus = "ON";
      Serial.println("Command: Motor ON");
      server.send(200, "text/plain", "Motor is now ON");
    });

    // 3. Turn Motor OFF
    server.on("/motor/off", HTTP_GET, []() {
      digitalWrite(MOTOR_PIN, LOW);
      motorStatus = "OFF";
      Serial.println("Command: Motor OFF");
      server.send(200, "text/plain", "Motor is now OFF");
    });

    server.begin();
    Serial.println("HTTP Server started");

  } else {
    Serial.println("WiFi Failed");
    display.println("WiFi Failed :(");
  }
  
  display.display();
  delay(2000); 
}

void loop() {
  server.handleClient();

  if (pms.read(data)) {
    currentPM25 = data.PM_AE_UG_2_5;
    currentPM10 = data.PM_AE_UG_10_0;

    // --- PRINT TO OLED ---
    display.clearDisplay();
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.print("Air Quality (ug/m3)");

    if(WiFi.status() == WL_CONNECTED) {
        display.fillCircle(124, 3, 2, WHITE); 
    }
    
    // Show PM2.5
    display.setTextSize(2);
    display.setCursor(10, 15);
    display.print("PM2.5: ");
    display.print(currentPM25);
    
    // Show Motor Status at bottom
    display.setTextSize(1);
    display.setCursor(0, 50);
    display.print("Dispenser: ");
    if(motorStatus == "ON") {
      display.setTextColor(BLACK, WHITE); // Inverted text for visibility
      display.print(" ON ");
      display.setTextColor(WHITE);
    } else {
      display.print("OFF");
    }

    display.display(); 
  }
}